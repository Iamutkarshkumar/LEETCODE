...
public:
    int solve(int start,int end,string& word,int k){
        ...
        for(int chars = 1; chars <= 26 && chars*k <= end-start+1; chars++){
            ...
            int i=start;
            int windowLen = chars*k;
            for(int j=start;j<=end;j++){
                char ch=word[j];
                freq[ch-'a']++;
                if(freq[ch-'a']==k) goodChar++;
                else if(freq[ch-'a']==k+1) goodChar--;

                if(j-i+1 > windowLen) { //Need to shift window right
                    if(freq[word[i] - 'a'] == k) goodChar--;
                    else if(freq[word[i] - 'a'] == k+1) goodChar++;
                    freq[word[i] - 'a']--;
                    i++;
                }
                if(goodChar == chars) result++;
            }
        }
        return result;
    }
