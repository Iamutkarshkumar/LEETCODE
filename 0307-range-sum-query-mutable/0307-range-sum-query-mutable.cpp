...
public:
    ...
    int rangeSumSegTree(int start,int end,int i,int l,int r){
        if(r<start or l>end) return 0;
        if(l>=start and r<=end) return segTree[i];
        int mid=l+(r-l)/2;
        return rangeSumSegTree(start,end,2*i+1,l,mid)+rangeSumSegTree(start,end,2*i+2,mid+1,r);
    }
    ...
    int sumRange(int left, int right) {
        return rangeSumSegTree(left,right,0,0,n-1);
    }
};
